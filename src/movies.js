...
function scoresAverage(moviesArray) {
  moviesArray = moviesArray.map((movie) => movie.score)
  ...
  const moviesArrayAverage = elementsSum / moviesArray.length
  const moviesArrayAverageRounded = Math.round(moviesArrayAverage * 100) / 100
  return moviesArrayAverageRounded
}
console.log(scoresAverage(movies))

// with reduce

function scoresAverageReduce(moviesArray) {
  const scoresArray = moviesArray.map((movie) => movie.score)

  const elementsSum = scoresArray.reduce(
    (acc, currentScore) => acc + currentScore,
    0
  )

  const scoreAverage = elementsSum / scoresArray.length
  const scoreAverageRounded = Math.round(scoreAverage * 100) / 100

  return scoreAverageRounded
}

console.log(scoresAverageReduce(movies))
...
function dramaMoviesScore(moviesArray) {
  ...
  const scoreSum = dramaArray.reduce((acc, movie) => acc + movie.score, 0)
  const dramaAverage = scoreSum / dramaArray.length
  const dramaAverageRounded = Math.round(dramaAverage * 100) / 100
...
function orderAlphabetically(moviesArray) {
  ...
  const newArray20 = moviesArray.slice(0, 20)

  const newArrayTitlesSorted = newArray20.map((movie) => movie.title)

  return newArrayTitlesSorted
}
...
//   const arrayMin = newArray => newArray.map((movie.duration) =>
//     hoursToMin(movie.duration.slice(0, 1), duration.slice(3, 4))
//   )
// }
